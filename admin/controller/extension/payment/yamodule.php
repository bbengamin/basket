...
class ControllerExtensionPaymentYamodule extends Controller {
	...
	public function sendmail (){
		...
		$message = $this->load->view('extension/yamodule/invoice_msg'.$end_tpl, $data);
	...
	public function invoice (){
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/payment/yamodule/invoice', 'token=' . $this->session->data['token'] . $url, true)
		);
		...
		$pagination->url = $this->url->link('extension/payment/yamodule/invoice', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$data['footer'] = $this->load->controller('common/footer');
		$end_tpl = (version_compare(VERSION, "2.2.0", '>='))?'':".tpl";
		$this->response->setOutput($this->load->view('extension/yamodule/invoice_list'.$end_tpl, $data));
	}
	public function test(){
		...
		$url = str_replace("http://","https://",HTTPS_CATALOG).'index.php?route=extension/payment/yamodule/callback';
        ...
        $data['footer'] = $this->load->controller('common/footer');
        $end_tpl = (version_compare(VERSION, "2.2.0", '>='))?"":".tpl";
        $this->response->setOutput($this->load->view('extension/yamodule/check'.$end_tpl, $data));
	}
