<?php
class ControllerExtensionFeedYamodule extends Controller {
	...
	public function sendStatistics()
	{
		...
		$this->load->language('extension/feed/yamodule');
		...
		switch($this->request->post['type_data'])
		{
			case 'kassa':
				...
				if(isset($this->request->post['ya_kassa_active']) && $this->request->post['ya_kassa_active'] == 1){
					$testUrl = $this->url->link('extension/payment/yamodule/test', 'token=' . $this->session->data['token'], 'SSL');
			...
			case 'metrika':
				...
				$this->load->model('extension/yamodule/metrika');
				...
				$this->model_extension_yamodule_metrika->processCounter($yaMetrika_number, $yaMetrika_token);
				break;
			case 'market':
	...
	public function initForm($array)
	{
		...
		$data['ya_kassa_check'] = str_replace("http://","https://",HTTPS_CATALOG).'index.php?route=extension/payment/yamodule/callback';
		$data['ya_kassa_aviso'] = str_replace("http://","https://",HTTPS_CATALOG).'index.php?route=extension/payment/yamodule/callback';
		...
		if ($this->config->get('config_secure'))
		{
			...
			$data['ya_p2p_linkapp'] = HTTPS_CATALOG.'index.php?route=extension/payment/yamodule/inside';
			$data['ya_market_lnk_yml'] = HTTPS_CATALOG.'index.php?route=extension/feed/yamarket';
		}
		else
		{
			...
			$data['ya_p2p_linkapp'] = HTTP_CATALOG.'index.php?route=extension/payment/yamodule/inside';
			$data['ya_market_lnk_yml'] = HTTP_CATALOG.'index.php?route=extension/feed/yamarket';
		}
		...
		$data['ya_metrika_callback'] = $this->url->link('extension/feed/yamodule/preparem', 'token='.$this->session->data['token'], 'SSL');
		...
		$data['ya_pokupki_callback'] = $this->url->link('extension/feed/yamodule/preparep', 'token='.$this->session->data['token'], 'SSL');
	...
	public function index()
	{
		...
		$data['breadcrumbs'][] = array(
			'href'      => $this->url->link('extension/feed/yamodule', 'token=' . $this->session->data['token'], 'SSL'),
			'text'      => $this->language->get('heading_title_ya'),
			'separator' => ' :: '
		);
		$this->tabMws($data);
		$data['action'] = $this->url->link('extension/feed/yamodule', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('extension/feed/yamodule'.$end_tpl, $data));
	}
	...
	public function install()
	{
		...
		$this->model_user_user_group->addPermission($this->user->getGroupId(), 'access', 'extension/payment/yamodule');
		$this->model_user_user_group->addPermission($this->user->getGroupId(), 'modify', 'extension/payment/yamodule');
		$this->model_user_user_group->addPermission($this->user->getGroupId(), 'access', 'extension/feed/yamodule');
		$this->model_user_user_group->addPermission($this->user->getGroupId(), 'modify', 'extension/feed/yamodule');
		...
		$this->model_user_user_group->addPermission($this->user->getGroupId(), 'access', 'extension/payment/test');
	}
	...
	public function changestatus(){
		...
		if ($order_info['customer_id'] == $this->config->get('yandexbuy_customer')){
			...
			$this->load->model('extension/yamodule/pokupki');
			$json = $this->model_extension_yamodule_pokupki->sendOrder($order_id, $order_status_id, $comment);
		}
		//
