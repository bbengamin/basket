...
*/
class ControllerExtensionFeedYamarket extends Controller {

	public function index()
	{
		$this->load->model('extension/yamodel/yamarket');
		$this->load->model('catalog/product');
		$this->load->model('tool/image');
		$this->load->model('localisation/currency');

		$categories = $this->model_extension_yamodel_yamarket->getCategories();
	...
	public function makeOfferCombination($data, $product, $shop_currency, $offers_currency, $decimal_place, $object)
	{
		...
		if (count($this->config->get('ya_market_color_options')))
			$colors = $this->model_extension_yamodel_yamarket->getProductOptions($this->config->get('ya_market_color_options'), $product['product_id']);
		if (count($this->config->get('ya_market_size_options')))
			$sizes = $this->model_extension_yamodel_yamarket->getProductOptions($this->config->get('ya_market_size_options'), $product['product_id']);
