...
class ControllerExtensionPaymentYamodule extends Controller
{
   ...
   const ORDERNUMBER = "orderNumber";
	...
	public function index() {
		...
		$data['p2p_action'] = $this->url->link('extension/payment/yamodule/yaredirect', '', 'SSL');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/yamodule.tpl')) {
	...
	public function yaredirect()
	{
		if (isset($_POST['payment-type']) && !empty($_POST['payment-type']))
		{
			if ($_POST['payment-type'] == 'wallet')
			{
				...
				$auth_url = $this->buildObtainTokenUrl($this->config->get('ya_p2p_idapp'), $this->url->link('extension/payment/yamodule/insidewallet', '', 'SSL'), $scope);
			...
			if($response->status == 'success')
			{
				...
				do{
					$process_options = array(
						...
						'instance_id' => $instance_id,
						'ext_auth_success_uri' => $this->url->link('extension/payment/yamodule/successcard', '', 'SSL'),
	...
	public function insidewallet()
	{
		...
		if (empty($code))
			$this->response->redirect($this->url->link('extension/payment/yamodule/yaredirect', '', 'SSL'));
		else
		{
			$response = $this->getAccessToken($this->config->get('ya_p2p_idapp'), $code, $this->url->link('extension/payment/yamodule/yaredirect', '', 'SSL'), $this->config->get('ya_p2p_pw'));
			$error = '';
			$token = '';
	...
	public function callback()
	{
		...
		$this->load->model('extension/yamodel/yamoney');
		$password = $this->config->get('ya_kassa_pw');
		//$this->model_extension_yamodel_yamoney->password2 = $this->config->get('ya_p2p_pw');
