    ...
    public function selectShipping() {
        ...
        if(isset($this->request->post['code']) && isset($this->session->data['shipping_methods'])){
            ...
            if(isset($this->session->data['shipping_methods'][$code[0]]['quote'][$code[1]])){
                ...
                $json['totals'] = '';
                foreach ($total_data as $total) {
                    $json['totals'] .= '<tr class="total-item"><td colspan="3" style="border:none;"> </td><td class="text-right"><strong>'.$total['title'].'</strong></td><td class="text-right">'.$this->currency->format($total['value']).'</td></tr>';
                }
